...
- **产出文件内容与分析**：
  ...
  - **分析**：与迁移前的图像相比，在这张图中，代表源域和目标域的数据点**显著地混合在了一起**。我们可以清晰地看到，不同颜色的簇（代表不同故障类型）是跨越了两个域的，例如，某个代表“内圈故障”的黄色簇中，既包含了源域的彩色点，也包含了目标域的'x'标记。这证明了TCA算法**未能成功**地消除了领域差异。
...
我们首先尝试了经典的基于特征变换的迁移方法——**迁移成分分析 (TCA)**。

- **方法**: TCA旨在学习一个共享的潜在子空间，通过最小化**最大均值差异 (MMD)**，来使得投影后两个域的数据分布尽可能地接近。
- **结果**: 可视化结果（如下图）清晰地表明，尽管我们尝试了多种策略（如半监督指导、调整`gamma`参数），TCA**无法有效融合**源域和目标域。 两个域的数据在变换后依然形成**明显分离**的簇，证明源域和目标域的差异过大，已经超出了TCA这类算法的能力范围。

#### 3.2 最终方案：领域对抗神经网络 (DANN)

鉴于TCA的失败，我们升级了迁移策略，采用了更强大的**领域对抗神经网络 (DANN)**。

- **方法原理**: DANN通过引入一个“**领域判别器**”与“**特征提取器**”进行对抗博弈。特征提取器努力学习对分类任务有用、但能“骗过”领域判别器的特征；领域判别器则努力分辨特征来自哪个域。当两者达到一个动态平衡，即判别器无法有效区分领域时，我们便得到了**领域不变特征**，从而实现了知识的有效迁移。
- **实现**:
  - 模型结构在 `src/models/clf/DANN/build.py` 中定义。
  - 实验配置和超参数在 `runs/transfer_dann.yaml` 中管理。
  - 整个流程由 `src/pipelines/dann_pipeline.py` 驱动。
- **结果与分析**:
  - **可视化展示**: 最终的t-SNE图（如图 `tsne_after_dann.png`）展示了DANN的卓越效果。源域（彩色点）和目标域（'x'点）的数据**实现了深度融合**，形成了跨领域的、按类别聚集的清晰结构。这证明DANN成功克服了巨大的领域差异。
  - **目标域标定**: 最终对目标域未知数据的标定结果保存在 `outputs/predictions/DANN/target_preds_dann.csv`，这份文件即为题目三的最终答案。
...
| 阶段 (Stage)            | 方案/修改 (Solution/Modification)                            | 核心思想 (Core Idea)                                         | 进步点与效果 (Improvement & Outcome)                         | 遗留问题 (Remaining Issues)                                  |
| ----------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **1. 初始方案**         | **TCA (迁移成分分析)**                                       | 使用传统的特征变换方法，将源域和目标域的数据拉近。           | 建立了一个可运行的基线模型，但t-SNE图显示领域对齐**完全失败**，两个域的数据“泾渭分明”。 | TCA的线性或核变换能力不足以处理源域和目标域之间巨大的、非线性的分布差异。 |
| **2. 第一次模型升级**   | **从 TCA 切换到 DANN**                                       | 引入更强大的**领域对抗神经网络 (DANN)**，通过对抗训练自动学习领域不变特征。 | 成功实现了**领域对齐**。t-SNE图上，源域和目标域的点开始混合，证明了DANN在混淆领域上的有效性。 | **分类结构完全丧失**。模型为了混淆领域，将所有类别的数据都搅乱在一起，形成了最初的“大脑”状图形，无法区分故障。 |
| **3. DANN初步优化**     | **优化DANN超参数** (`feature_extractor_arch`, `epochs`, `lambda`) | 调整MLP网络结构和训练参数，试图在对齐领域的同时保留分类信息。 | t-SNE图的**整体轮廓开始变化**，但内部依然混乱。证明单纯调参无法解决根本问题。 | MLP作为特征提取器，其能力不足以在混淆领域的同时，学习到具有区分性的故障特征。 |
| **4. 第二次模型升级**   | **特征提取器: MLP -> 一维CNN**                               | 认识到手工特征不适合MLP，转而使用更适合处理序列数据的**一维卷积神经网络(1D-CNN)作为特征提取器。 | 质的飞跃。t-SNE图首次出现了按颜色（类别）聚集的趋势**。证明了CNN架构在捕捉信号特征局部模式上的优越性。 | 聚类效果依然不理想，类别间边界模糊，簇内松散。“Normal”和“InnerRace”等类别未能有效分离。 |
| **5. 第三次模型升级**   | **特征提取器: 一维CNN -> MHDCNN**                            | 借鉴论文思想，实现更先进的**多尺度混合空洞卷积网络(MHDCNN)**，以期获得更强的特征提取能力。 | t-SNE图的**聚类轮廓变得更加清晰**，颜色聚集的区域性更明显。证明MHDCNN确实比简单CNN更能捕捉高阶模式。 | 整体依然是“大脑”状，**分类性能没有得到根本性改善**。这强烈暗示了问题的根源可能不在模型，而在更上游的环节。 |
| **6. 引入数据均衡策略** | **处理数据不平衡问题** (在损失函数中加入类别权重)            | 识别到源域数据中“Normal”类别样本极度稀缺的问题，通过**加权损失函数**强迫模型关注小样本类别。 | 理论上这是至关重要的一步，能够显著**提升模型对小样本类别的识别能力**，防止模型在训练中忽略它们。 | 尽管策略正确，但t-SNE图改善有限。这说明即使模型被强迫去学习“Normal”类，但输入的特征本身可能就没有提供足够的信息让它去学。 |
| **7. 根本原因诊断**     | **识别到“手工特征与深度学习的八字不合”**                     | 经过系统性排查，最终定位到核心矛盾：我们一直在用为传统机器学习（如XGBoost）设计的**低维统计特征向量(21维)**，来训练一个为高维结构化数据（如图像、原始序列）设计的**深度卷积网络**。 | **认知上的巨大进步**。我们终于理解了模型“英雄无用武之地”的根本原因：输入的特征维度过低，且丢失了原始信号的结构信息，导致CNN无法发挥其自动学习复杂模式的真正优势。 | 之前的所有努力都是在用一个强大的工具处理错误的“原材料”。     |
| **8. 最终解决方案**     | **生成时频图 (Spectrogram)**，**将一维信号转为二维图像**     | **回归本源**。放弃手工统计特征，将每个信号窗口转换为一张保留了时域和频域信息的**二维时频图**。同时，将模型架构彻底升级为处理图像的**2D-CNN**。 | 这是**最有希望彻底解决问题**的方案。它完美地匹配了**数据表征 (二维图像)** 和 **模型架构 (2D-CNN)**，让深度学习模型可以直接从最接近原始数据的结构化信息中进行端到端的学习。 | 实施过程相对复杂，需要重构特征提取和数据加载流水线，并且GPU资源对于加速这一过程至关重要。 |